...
- ✅ `lib/l10n/app_localizations.dart` + `app_localizations_en.dart` + `app_localizations_pl.dart` (NEW) — generated by `flutter gen-l10n`
- ✅ `l10n.yaml` (NEW) — ARB source directory and output configuration
- ✅ `lib/presentation/providers/app_locale_provider.dart` (NEW) — `AppLocaleProvider` ChangeNotifier: loads persisted locale from SharedPreferences key `app_language` on startup, exposes `setLocale(Locale)` with no-op guard for same locale
- ✅ `lib/main.dart` (MODIFIED) — `AppLocaleProvider` created and loaded before `runApp`; provided at root via `ChangeNotifierProvider.value`; `FriendsheetApp` reads locale via `context.watch<AppLocaleProvider>()` and passes to `MaterialApp`
- ✅ `lib/presentation/screens/settings_screen.dart` (MODIFIED) — "App Language" tile with `Icons.language`; `_showLanguageDialog` with two `RadioListTile` options (English / Polski); immediate locale switch on selection; `_currentLanguageName` helper
- ✅ All `lib/presentation/` screens and widgets (MODIFIED) — hardcoded strings replaced with `AppLocalizations.of(context)!.keyName` calls (37 files total)
- ✅ `pubspec.yaml` (MODIFIED) — `flutter_localizations` SDK package added; `generate: true` in `flutter:` section
- ✅ `test/presentation/providers/app_locale_provider_test.dart` (NEW) — 6 tests: default locale, loadSavedLocale (no value / with stored pl), setLocale (update + persist), no-op guard, full cycle
- ✅ All 40 existing widget test files (MODIFIED) — `AppLocalizations.delegate`, `GlobalMaterialLocalizations.delegate`, `GlobalWidgetsLocalizations.delegate` added to every `MaterialApp` wrapper so tests resolve `AppLocalizations.of(context)` without null
- ✅ 1044 Flutter tests passing (+6 new tests)
