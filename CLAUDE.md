...
6. Before listing files in the proposed commit, run `git status` to capture all modified files — including those regenerated by build_runner or reformatted by dart format. Then provide a proposed commit:
   ```powershell
   git status
