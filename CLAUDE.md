...
1. Run `dart format .` — fix all formatting issues (CI runs `dart format --set-exit-if-changed .`)
2. Run `flutter analyze` — fix all issues
3. Run `flutter test` — all must pass
4. List the Manual Verification steps the user should perform on a real device/emulator (write them in Polish)
5. Wait — do NOT suggest /qa until the user confirms manual verification passed
6. Only after confirmation: run `git status` to capture all modified files — including those regenerated by build_runner or reformatted by dart format. Then provide a proposed commit:
   ```powershell
   git status
...
7. After the commit is confirmed by the user: "Implementation committed. Run /qa."
