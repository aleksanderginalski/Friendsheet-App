...
- `AIChatProvider` ✅ — ChangeNotifier managing chat session: `BuddyChatMode`-based initialization paths (`freeQuery`, `meetingNotes`, `meetingNotesList`, `birthdayList`, `birthdayWishes`); `pendingActions: List<BuddyAction>?` for in-chat action buttons; `handleAction()` continues flow after user tap; `_initBirthdayWishes()` streams Dart stats + AI wish; streaming accumulation, pseudonym back-translation, retry logic (`lib/presentation/ai_chat/ai_chat_provider.dart`)
- `BuddyWidgetProvider` ✅ — ChangeNotifier for HomeScreen floating widget; detects upcoming birthdays via `PersonRepository` (days-until computation, 5-day urgency threshold); fetches top-3 `getRecentMeetingsWithoutNotes` (60-day window); exposes `urgentBirthdayPersons`, `daysUntilBirthday`, `upcomingBirthdayInfo`, `suggestedMeetings`, `isExpanded`; created and owned by `MainScreen` (`lib/presentation/providers/buddy_widget_provider.dart`)
- `BuddyWidget` ✅ — purely presentational floating widget; `Column([bubble, icon])` layout for correct hit-test bounds; 2-button layout: "💾 Save Your Memories" + birthday CTA variant; `_TailPainter` CustomPainter triangle; callbacks route to `AIChatScreen` in mode-specific flows (`lib/presentation/widgets/buddy_widget.dart`)
