...
- `AppLocaleProvider` ✅ — ChangeNotifier at app root; loads persisted locale from SharedPreferences key `app_language` on startup; `setLocale(Locale)` no-ops if locale unchanged, otherwise updates and persists; provided via `ChangeNotifierProvider.value` in `main.dart`; `FriendsheetApp` reads locale via `context.watch<AppLocaleProvider>()` (US-116) (`lib/presentation/providers/app_locale_provider.dart`)

**Localization infrastructure (US-116):**
- `lib/l10n/app_en.arb` — English source strings (~340 keys); all UI-visible text except user content, AI prompts, and debug strings
- `lib/l10n/app_pl.arb` — Polish translations; informal "ty" form; ALL CAPS dialog buttons; Title Case AppBar titles
- `lib/l10n/app_localizations.dart` + `app_localizations_en.dart` + `app_localizations_pl.dart` — generated by `flutter gen-l10n`; committed to repo (consistent with project policy for generated files)
- `l10n.yaml` — arb-dir: `lib/l10n`, template: `app_en.arb`, output: `app_localizations.dart`
- Locale switch takes effect immediately without app restart — `MaterialApp.locale` bound to `AppLocaleProvider.locale` via `context.watch`
