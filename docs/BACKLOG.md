...
- [ ] **TASK-085.2:** Create `AIConsentRepository` (SharedPreferences, key: `ai_consent_granted`, methods: `hasGrantedConsent` / `grantConsent`) — 0.5h
- [ ] **TASK-085.3:** Add consent gate in `SettingsScreen._openAISettings()` — check consent; if not granted push `AIConsentScreen` which on agree calls `grantConsent()` then navigates to `AISettingsScreen`; if already granted push `AISettingsScreen` directly — 0.5h
- [ ] **TASK-085.4:** Update `docs/privacy.md` with section 2.5 (AI data processing) — 0.5h
