$descriptions = @{
    ...
    "lib/presentation/widgets/year_stepper.dart"                                = "YearStepper — StatefulWidget; 5-slot row layout: [←] [prev year dimmed] [active year bold/primary] [next year dimmed] [→]; onHorizontalDragUpdate accumulates displacement (_kPixelsPerYear=50.0) → updates _previewYear (italic, 60% opacity); onHorizontalDragEnd commits via onYearChanged, onHorizontalDragCancel resets without commit; LinearProgressIndicator track shows position in year range, hidden when minYear==maxYear; arrows and single-swipe behavior preserved (US-027, US-071, US-081)"    # --- tests ---
