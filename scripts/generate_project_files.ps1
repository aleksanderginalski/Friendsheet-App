$descriptions = @{
    ...
    "lib/presentation/widgets/activity_breakdown_widget.dart"               = "Animated vertical bar chart — Stack + absolute positioning, ChartColors gradient per categoryId, delta % indicator (▲/▼/NEW), ⚙️ visibility dialog trigger, auto-select top 10 logic; _lastTargetLeft fix for stationary bar animation (US-028, US-048, US-049, US-063)"
    ...
    "lib/presentation/widgets/interaction_distribution_widget.dart"         = "Animated bar chart showing meeting weight per person — yearly/cumulative toggle, isLoading inline spinner (widget always stays in tree), info icon (>100% explanation), ⚙️ visibility dialog, _lastTargetLeft animation architecture, ChartColors gradient per personId (US-030, US-051, US-063)"
