$descriptions = @{
    ...
    # US-077: updated — waiting_room.png in empty state (_buildEmptyState) and as last item in _buildCandidateList (itemCount = items.length + 1); only shown when list non-empty
    ...
    # US-077: updated — drawer header green area now shows drawer_icon.png (Image.asset, height 80) below 'Friendsheet' label
    ...
    # US-077: updated — meeting item tiles padding changed from symmetric(horizontal: 8, vertical: 2) to fromLTRB(24, 2, 8, 2); adds 16dp extra left indent vs _MonthHeader
